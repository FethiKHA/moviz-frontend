...
function Home() {
  ...
  useEffect(() => {
    fetch('http://localhost:https://moviz-backend-585.vercel.app/movies')
 ...
 */
  const movies = moviesData.map((movie, i) => {
    ...
    return (<Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={movie.title} overview={movie.overview} poster={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} voteAverage={movie.vote_average} voteCount={movie.vote_count} />);
